#!/usr/bin/env python3
"""Train Newhex joystick PPO sequentially across robot asset variants.

Usage:
  uv run scripts/sweep_newhex_joystick_robots.py
  uv run scripts/sweep_newhex_joystick_robots.py --name-template 'g001_o{index:03d}' --start 30 --end 49
"""
...
JOYSTICK_CFG = REPO_ROOT / "src" / "unilab" / "envs" / "locomotion" / "newhex" / "joystick.py"
NEWHEX_ASSETS = REPO_ROOT / "src" / "unilab" / "assets" / "robots" / "newhex"
DEFAULT_CSV_PATH = REPO_ROOT / "newhex_joystick_sweep_results.csv"
TRAIN_COMMAND = ("uv", "run", "train", "--algo", "ppo", "--task", "newhex_joystick_flat", "--sim", "motrix")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--name-template",
        default="g060_o{index:03d}",
        help="Robot directory template. Use {index} for the numeric suffix.",
    )
    parser.add_argument(
        "--joystick-cfg",
        type=Path,
        default=JOYSTICK_CFG,
        help="Path to src/unilab/envs/locomotion/newhex/joystick.py.",
    )
    parser.add_argument(
        "--assets-root",
        type=Path,
    ...
    parser.add_argument(
        "--restore-original",
        action="store_true",
        help="Restore the original joystick robot directory after the sweep finishes.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def _read_current_robot(joystick_cfg: Path) -> tuple[str, str]:
    text = joystick_cfg.read_text()
    matches = list(MODEL_FILE_RE.finditer(text))
    if len(matches) != 1:
        raise ValueError(
            f"expected exactly one Newhex scene.xml model_file in {joystick_cfg}, found {len(matches)}"
        )
    return text, matches[0].group("robot")


def _write_robot(joystick_cfg: Path, robot_name: str) -> None:
    text, _ = _read_current_robot(joystick_cfg)

    def replace(match: re.Match[str]) -> str:
        return f"{match.group('prefix')}{robot_name}{match.group('suffix')}"

    joystick_cfg.write_text(MODEL_FILE_RE.sub(replace, text, count=1))
...
def _run_training(robot_name: str) -> tuple[int, int | None, float | None]:
    print(f"[newhex-sweep] training {robot_name}: {' '.join(TRAIN_COMMAND)}", flush=True)
    env = os.environ.copy()
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    joystick_cfg = args.joystick_cfg.expanduser().resolve()
    assets_root = args.assets_root.expanduser().resolve()
    csv_path = args.csv_path.expanduser().resolve()
    _check_conda_env(args.conda_env)
    _, original_robot = _read_current_robot(joystick_cfg)
    ...
    try:
        for index in range(args.start, args.end + 1):
            ...
            print(f"[newhex-sweep] set joystick robot: {robot_name}", flush=True)
            if not args.dry_run:
                _write_robot(joystick_cfg, robot_name)
                ...
                if mean_reward is None:
                    print(
                        f"[newhex-sweep] warning: no Mean reward found in logs for {robot_name}",
                        flush=True,
                    )
                else:
                    print(
                        f"[newhex-sweep] recorded {robot_name}: "
                        f"iteration={iteration}, mean_reward={mean_reward}",
                        flush=True,
                    )
            else:
                print(f"[newhex-sweep] dry-run command: {' '.join(TRAIN_COMMAND)}", flush=True)
                print(f"[newhex-sweep] dry-run csv path: {csv_path}", flush=True)
                exit_code = 0

            if exit_code != 0:
                failures.append((robot_name, exit_code))
                print(
                    f"[newhex-sweep] training failed for {robot_name} with exit code {exit_code}",
                    flush=True,
                )
        ...
        if args.restore_original and not args.dry_run:
            print(f"[newhex-sweep] restoring original joystick robot: {original_robot}", flush=True)
            _write_robot(joystick_cfg, original_robot)

    if failures:
        print("[newhex-sweep] completed with failures:", flush=True)
        for robot_name, exit_code in failures:
            print(f"  {robot_name}: exit code {exit_code}", flush=True)
        return 1

    print("[newhex-sweep] completed successfully", flush=True)
    return 0
