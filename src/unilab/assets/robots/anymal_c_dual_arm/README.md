...
`anymal_c_dual_arm.xml` combines the ANYmal C MJX model with two six-axis
Airbot Play arms. Use `scene_dual_arm.xml` to load the robot with a ground
plane:

```bash
python view_mjcf.py robot/anymal_c_dual_arm/scene_dual_arm.xml
```

The left and right arm mounts are at `y=0.10 m` and `y=-0.10 m`,
respectively, and both are `0.14 m` above the ANYmal base frame.
