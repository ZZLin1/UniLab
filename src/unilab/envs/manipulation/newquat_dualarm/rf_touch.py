...
SUPPORT_FOOT_INDICES = np.asarray([0, 1, 2, 3], dtype=np.int32)


@dataclass
class InitState:
    pos: list[float] = field(default_factory=lambda: [0.0, 0.0, 0.278])
...
@registry.envcfg("NewquatDualArmRFTouch")
@dataclass
class NewquatDualArmRFTouchCfg(NewquatDualArmBaseCfg):
...
class NewquatDualArmRFTouchDRProvider(DomainRandomizationProvider):
    ...
    def build_reset_observation(self, env: Any, env_ids: np.ndarray, info_updates: dict[str, Any]) -> dict[str, np.ndarray]:
        ...
        info_updates["commands"] = env._compute_commands_for_envs(env_ids)
        ...
        return cast(dict[str, np.ndarray], env._compute_obs(info_updates, gyro, gravity, dof_pos, dof_vel))


@registry.env("NewquatDualArmRFTouch", sim_backend="mujoco")
@registry.env("NewquatDualArmRFTouch", sim_backend="motrix")
class NewquatDualArmRFTouchEnv(NewquatDualArmBaseEnv):
    _cfg: NewquatDualArmRFTouchCfg

    def __init__(self, cfg: NewquatDualArmRFTouchCfg, num_envs: int = 1, backend_type: str = "mujoco"):
        ...
        super().__init__(cfg, backend, num_envs, policy_action_dim=MODEL_ACTION_DIM)
        ...
        self._rf_body_ids = np.asarray([self._backend.get_body_id(cfg.load.body_name)], dtype=np.int32)
        ...
        self.feet_force = np.zeros((num_envs, len(cfg.sensor.feet_force), 3), dtype=get_global_dtype())
        self._support_foot_start_pos_w = np.zeros((num_envs, len(SUPPORT_FOOT_INDICES), 3), dtype=get_global_dtype())
        self._last_dof_vel_for_acc = np.zeros((num_envs, MODEL_ACTION_DIM), dtype=get_global_dtype())
        self._init_reward_functions()
        self._init_domain_randomization(NewquatDualArmRFTouchDRProvider())

    @property
    def obs_groups_spec(self) -> dict[str, int]:
        return {"obs": 81, "critic": 81}
    ...
    def get_playback_extra_data(self) -> dict[str, np.ndarray]:
        return {
            ...
            "box_sizes": np.broadcast_to(self._target_box_size(), (self._num_envs, 3)).copy(),
        }
    ...
    def apply_action(self, actions: np.ndarray, state: NpEnvState) -> np.ndarray:
        ...
        ctrl = exec_actions * self._cfg.control_config.action_scale + self.default_angles_full
        self._apply_rf_load()
    ...
    def update_state(self, state: NpEnvState) -> NpEnvState:
        ...
        state.info["commands"] = self._compute_commands_for_envs(np.arange(self._num_envs, dtype=np.int32))
        ...
        return state.replace(obs=obs, reward=reward, terminated=gravity[:, 2] <= 0.5)
    ...
    def _sample_targets(self, env_ids: np.ndarray, base_xy: np.ndarray) -> None:
        ...
        samples = np.random.uniform(low=low, high=high, size=(len(env_ids), 3)).astype(get_global_dtype())
    ...
    def _sample_rf_load(self, num_reset: int) -> np.ndarray:
        ...
        return np.asarray(np.random.uniform(float(low), float(high), size=(num_reset,)), dtype=get_global_dtype())
    ...
    def _compute_commands_for_envs(self, env_ids: np.ndarray) -> np.ndarray:
        ...
        ee_pos = self.right_endpoint_world_pos()[rows]
        base_quat = self._backend.get_base_quat()[rows]
        return np.asarray(np_quat_apply_inverse(base_quat, self._target_pos_w[rows] - ee_pos), dtype=get_global_dtype())

    def _compute_obs(self, info: dict[str, Any], gyro, gravity, dof_pos, dof_vel) -> dict[str, np.ndarray]:
        noise_cfg = self._cfg.noise_config
        diff = self._obs_noise(dof_pos - self.default_angles_full, noise_cfg.scale_joint_angle)
        ...
        obs = np.concatenate([gyro, -gravity, diff, dof_vel, last_actions, command], axis=1, dtype=get_global_dtype())
    ...
    def _compute_reward(self, info: dict, linvel, gyro, dof_pos, dof_vel) -> np.ndarray:
        ctx = RewardContext(
            ...
            default_angles=self.default_angles_full,
    ...
    def _refresh_feet_buffers(self, rows: np.ndarray | None = None) -> None:
        ...
        for i, name in enumerate(self._cfg.sensor.feet_pos):
            self.feet_pos[target, i, :] = self._backend.get_sensor_data(name)[target]
        for i, name in enumerate(self._cfg.sensor.feet_vel):
            self.feet_vel[target, i, :] = self._backend.get_sensor_data(name)[target]
        self.feet_force[target, :, :] = 0.0
        for i, name in enumerate(self._cfg.sensor.feet_force):
            values = np.asarray(self._backend.get_sensor_data(name)[target], dtype=get_global_dtype())
            if values.ndim == 2 and values.shape[1] > 1:
                strength = np.linalg.norm(values, axis=1)
            else:
                strength = np.ravel(values)
            self.feet_force[target, i, 2] = strength

    def _reward_base_lin_vel_zero(self, ctx: RewardContext) -> np.ndarray:
        return np.exp(-np.sum(np.square(ctx.linvel), axis=1) / self._reward_cfg.tracking_sigma)

    def _reward_base_ang_vel_zero(self, ctx: RewardContext) -> np.ndarray:
        return np.exp(-np.sum(np.square(ctx.gyro), axis=1) / self._reward_cfg.tracking_sigma)
    ...
    def _reward_support_feet_still(self, ctx: RewardContext) -> np.ndarray:
        ...
        return np.exp(-np.sum(np.square(delta[..., :2]), axis=(1, 2)) / self._reward_cfg.support_still_sigma)
    ...
    def _reward_support_feet_vel(self, ctx: RewardContext) -> np.ndarray:
        ...
        return np.exp(-speed_sq / self._reward_cfg.support_vel_sigma)
    ...
    def _reward_support_feet_contact(self, ctx: RewardContext) -> np.ndarray:
        ...
        return np.mean(contact.astype(get_global_dtype()), axis=1)
    ...
    def _reward_rf_target(self, ctx: RewardContext) -> np.ndarray:
        del ctx
        distance = np.linalg.norm(self.right_endpoint_world_pos() - self._target_pos_w, axis=1)
    ...
    def _estimate_pd_torques(self, info: dict[str, Any], dof_pos: np.ndarray, dof_vel: np.ndarray) -> np.ndarray:
        actions = np.asarray(info.get("current_actions", np.zeros((dof_pos.shape[0], MODEL_ACTION_DIM))), dtype=get_global_dtype())
        targets = actions * self._cfg.control_config.action_scale + self.default_angles_full
        gains = build_position_gains(self._cfg.control_config)
        return (gains["kp"] * (targets - dof_pos) - gains["kd"] * dof_vel).astype(get_global_dtype())

    def _target_box_center_w(self) -> np.ndarray:
        low = np.asarray(self._cfg.target_sampling.low, dtype=get_global_dtype())
        high = np.asarray(self._cfg.target_sampling.high, dtype=get_global_dtype())
        center = np.zeros((self._num_envs, 3), dtype=get_global_dtype())
        center[:, :2] = self._backend.get_base_pos()[:, :2] + 0.5 * (low[:2] + high[:2])
