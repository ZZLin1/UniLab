...
from unilab.envs.locomotion.hex4.base import Hex4BaseCfg, Hex4BaseEnv
...
@dataclass
class Hex4DomainRandConfig(DomainRandConfig):
...
@registry.envcfg("Hex4JoystickFlat")
@dataclass
class Hex4JoystickFlatCfg(Hex4BaseCfg):
    scene: SceneCfg = field(
        default_factory=lambda: SceneCfg(
            model_file=str(ASSETS_ROOT_PATH / "robots" / "hex4" / "scene_flat.xml")
        )
    )
    ...
    domain_rand: Hex4DomainRandConfig = field(default_factory=Hex4DomainRandConfig)
    terrain_curriculum: TerrainCurriculumCfg = field(default_factory=TerrainCurriculumCfg)


class Hex4JoystickDomainRandomizationProvider(LocomotionDRProvider):
...
@registry.env("Hex4JoystickFlat", sim_backend="mujoco")
@registry.env("Hex4JoystickFlat", sim_backend="motrix")
class Hex4WalkTask(Hex4BaseEnv):
    _cfg: Hex4JoystickFlatCfg

    def __init__(self, cfg: Hex4JoystickFlatCfg, num_envs=1, backend_type="mujoco"):
        ...
        self._init_domain_randomization(Hex4JoystickDomainRandomizationProvider())
    ...
    def obs_groups_spec(self) -> dict[str, int]:
        # gyro(3) + gravity(3) + diff(24) + dof_vel(24) + action(24) + cmd(3) + phase(6) = 87
        return {"obs": 87, "critic": 90} 
